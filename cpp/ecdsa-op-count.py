...
uiShiftRight1 = funcOvh + 6 + 7*8
uiReciprocal  = funcOvh + 4*uiCopy + uiAdd + uiShiftRight1 + uiEquals + uiReplace + 512*(11 + 2*uiSwap + 2*uiAdd + 2*uiSubtract + 2*uiShiftRight1 + uiLessThan)
...
fiAdd        = funcOvh + 3 + uiLessThan + uiAdd + uiSubtract
fiSubtract   = funcOvh + 1 + uiAdd + uiSubtract
fiMultiply2  = funcOvh + 3 + uiLessThan + uiSubtract + uiShiftLeft1
...
cpAdd       = funcOvh + 10 + 10*fiCopy + 2*fiEquals + fiAdd + 5*fiSubtract + 13*fiMultiply + 2*fiSquare + cpCopy + 4*cpReplace + 2*cpIsZero + cpTwice
