...
class Base58Check final {
	...
	public: static void pubkeyHashToBase58Check(const uint8_t pubkeyHash[Ripemd160::HASH_LEN], char outStr[35]);
	...
	public: static bool pubkeyHashFromBase58Check(const char *addrStr, uint8_t outPubkeyHash[Ripemd160::HASH_LEN]);
	...
	private: static void bytesToBase58Check(uint8_t *data, size_t dataLen, char *outStr);
	...
	private: static bool base58CheckToBytes(const char *inStr, uint8_t *outData, size_t outDataLen);
	...
	private: static bool isZero(const uint8_t *x, size_t len);
	
	
	// Returns the given bigint modulo 58. Not constant-time.
	private: static uint8_t mod58(const uint8_t *x, size_t len);
	
	
	// Computes the quotient y = floor(x / 58). Not constant-time.
	private: static void divide58(const uint8_t *x, uint8_t *y, size_t len);
	...
	private: static bool addUint8(uint8_t *x, uint8_t y, size_t len);
	...
	private: static bool multiply58(uint8_t *x, size_t len);
