 ...
 * 
 * This class contains fields and methods common to both FieldInt and Uint256.
 * Each subclass chooses which methods to expose, and may implement higher-level methods.
 */
public class AbstractInt256 {
	...
	// Constructs an integer initialized to zero. Constant-time.
	protected AbstractInt256() {
		value = new int[8];
	}
	
	
	// Constructs an integer copying the given object. Constant-time.
	protected AbstractInt256(AbstractInt256 other) {
		value = other.value.clone();
	}
	
	
	// Constructs an integer from the given 64-character hexadecimal string. Not constant-time.
	protected AbstractInt256(String s) {
		this();
	...
	// Constructs an integer from the given 32 bytes encoded in big-endian. Constant-time.
	protected AbstractInt256(byte[] b, int off) {
		this();
	...
	protected int add(AbstractInt256 other, int mask) {
	...
	protected int subtract(AbstractInt256 other, int mask) {
		if (other == this || ((mask + 1) >>> 1) != 0)
			throw new IllegalArgumentException();
		long otherMask = LONG_MASK & mask;
		int borrow = 0;
	...
	protected void shiftRight1(int mask) {
		assert(((mask + 1) >>> 1) == 0);
		int cur = value[0];
	...
	protected void reciprocal(AbstractInt256 modulus, AbstractInt256 zero, AbstractInt256 one) {
		// Extended binary GCD algorithm
		if (modulus == this || (modulus.value[0] & 1) == 0)
			throw new IllegalArgumentException();
		AbstractInt256 x = new AbstractInt256(modulus);  // Must be odd
		AbstractInt256 y = new AbstractInt256(this);  // Odd or even, and must be less than x
		AbstractInt256 a = new AbstractInt256(zero);
		AbstractInt256 b = new AbstractInt256(one);
		AbstractInt256 halfModulus = new AbstractInt256(modulus);
		halfModulus.add(one, -1);
		halfModulus.shiftRight1(-1);
	...
	protected void replace(AbstractInt256 other, int mask) {
		assert(((mask + 1) >>> 1) == 0);
		for (int i = 0; i < value.length; i++)
	...
	protected void swap(AbstractInt256 other, int mask) {
		assert(((mask + 1) >>> 1) == 0);
		for (int i = 0; i < value.length; i++) {
			int x = this .value[i];
			int y = other.value[i];
	...
	public boolean equals(Object obj) {
		return obj == this || obj instanceof AbstractInt256 && Arrays.equals(value, ((AbstractInt256)obj).value);
	}
	
	
	// Returns whether this number is less than, equal to, or greater than the given number. Not constant-time.
	protected int compareTo(AbstractInt256 other) {
		for (int i = value.length - 1; i >= 0; i--) {
	...
	protected int equalTo(AbstractInt256 other) {
		int diff = 0;
		for (int i = 0; i < value.length; i++)
	...
	protected int lessThan(AbstractInt256 other) {
		int result = 0;
		for (int i = 0; i < value.length; i++) {
			int x = this .value[i];
			int y = other.value[i];
